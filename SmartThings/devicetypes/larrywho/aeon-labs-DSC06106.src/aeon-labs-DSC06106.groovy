 ...
 */
metadata {
    definition (name: "Aeon Labs DSC06106", namespace: "larrywho", author: "Larry W") {
	capability "Switch"
 ...
 */
def refresh() {
    if (state.debug) log.debug "in refresh()"
    delayBetween([
	zwave.basicV1.basicGet().format(),
	zwave.switchBinaryV1.switchBinaryGet().format(),
...
def configure() {
    ...
    ***************************************************************/
	...
	zwave.configurationV1.configurationSet(parameterNumber: 0x5B, size: 2, scaledConfigurationValue: 0).format(),	//Minimum change in wattage for a REPORT to be sent (Valid values 0 - 60000)
	zwave.configurationV1.configurationSet(parameterNumber: 0x5C, size: 1, scaledConfigurationValue: 0).format(),	//Minimum change in percentage for a REPORT to be sent (Valid values 0 - 100)
